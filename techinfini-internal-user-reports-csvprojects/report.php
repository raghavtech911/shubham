<?php
	...
	if(isset($_POST['singlebtn'])){
		$SelectedProject = $_POST['project_select'];
		$querytogetproject = 'SELECT project, SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time  
							  FROM csv_info 
							  WHERE project="'.$SelectedProject.'"';
			if($res = mysqli_query($con, $querytogetproject)){
				$fileName = 'singleproject.csv';
		  		$num = 0;
		  		ob_end_clean();
				while ($row = mysqli_fetch_assoc($res)){
		        	...
		        	$prod[$num]['time']      = $time;		        
 					 ...
 					 $q = "SELECT distinct(user_name) 
				  	 	  FROM csv_info 
				  	 	  WHERE project='".$row['project']."'";
				  	 		if($r = mysqli_query($con, $q)){
				  	 			while ($ro = mysqli_fetch_assoc($r)){
				  	 				$pro[$num]['user_name'] = $ro['user_name']; 
							 		$sql1= "SELECT SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time 
							 				FROM csv_info 
							 				WHERE project='".$row['project']."'AND user_name='".$ro['user_name']."'";
				...
				$f = fopen('php://output', 'w');
	 		 	...
	 		 	fputcsv($f, array('Project Name','Total Time in hr'));
	 		 	foreach($prod as $product) {
		 	 		   fputcsv($f, $product);
				 	}
				fputcsv($f, array('',''));
	 		 	fputcsv($f, array('User','Total Time in hr'));
	 		 		foreach($pro as $p) {
		 	 		   fputcsv($f, $p);
				 	}
		...
		if ($result = mysqli_query($con, $sql)){
		  	...
		  	while ($row = mysqli_fetch_assoc($result)){
		        ...
		        $num++;
	 			...
	 			fputcsv($fp, array('Project Name','Total Time in hr'));
	 			foreach($prod as $product) {
		 			   fputcsv($fp, $product);
					}
			 	fclose($fp);
								...
								if ($result = mysqli_query($con, $sql)){
						  		...
						  		while ($row = mysqli_fetch_assoc($result)){
								        ...
								        $num++;
							 			...
							 			header('Content-Disposition: attachment; filename="' . $fileName . '"');
						  				fputcsv($fp, array('From: '.$start_date.'  To: '.$end_date));
							 			fputcsv($fp, array('Project Name','Jan','Feb','Mar','April','May','jun','jul','Aug','Sept','Oct','Nov','Dec','Total in Hr'));
							 			foreach($prod as $product) {
								 			   fputcsv($fp, $product);
										}
							 		fclose($fp);
...
<head>
	...
	<script
 	...
 	<style type="text/css">
	 	select#format_select,  select#project_select{
...
<div class="container-fluid" > 
	...
	<form action="" method="POST">
		<label style="margin-left: 60px;">Start Date:  </label> <input id="startdate" type="date" name="startdate" value="2017-09-01">
		...
		<label>Select Project</label>
			...
			<option value="err">select any project</option>
		<?php
			$query = "SELECT distinct(project) FROM csv_info";
...
<?php
	...
	#----------------------For single project-------------------------------
	if(isset($_POST['project_select'])){
		if($_POST['project_select'] !== 'err'){
		$SelectedProject = $_POST['project_select'];
		$querytogetproject = 'SELECT project, SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time  FROM csv_info WHERE project="'.$SelectedProject.'"';
		if($res = mysqli_query($con, $querytogetproject)){
			echo "<div class='container'>";
			echo "<input type='submit' name='singlebtn' value='Export To CSV' class='btn btn-info' style='float:right; margin-bottom: 7px;'> <br>
			...
			";
				while ($row = mysqli_fetch_assoc($res)){
					...
					$time = sprintf("%02d:%02d", $H, $i);
				  	...
				  	$q = "SELECT distinct(user_name) FROM csv_info WHERE project='".$row['project']."'
				  	";
				  	if($r = mysqli_query($con, $q)){
				  	 	echo "<td colspan='4'><table width='100%' bgcolor='#c0edea'>";
				  	 	echo "<tr bgcolor='#6b98e0'>
				 		 <th>User</th>
				 		 <th>Total Time <small> in Hr</small></th>
				 		 </tr>";
				  		while ($ro = mysqli_fetch_assoc($r)){
				  				echo "<tr>";
								 echo "<td>".$ro['user_name']."</td>";
								$sql1= "SELECT SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time FROM csv_info WHERE project='".$row['project']."'AND user_name='".$ro['user_name']."'";
								if ($result1 = mysqli_query($con, $sql1)){
									while ($ro = mysqli_fetch_assoc($result1)){
											$seconds = $ro['time'];
											$H = floor($seconds / 3600);
											$i = ($seconds / 60) % 60;
											$time1 = sprintf("%02d:%02d", $H, $i);
										echo "<td>".$time1."</td>";
									}
								}
								 //echo "</tr>";
...
#------------------------------------------FOR NONE VIEW -----------------------------
	...
	if($_POST['format_select'] == 'none'){
		 ...
		 $sql = "SELECT id, project, SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time FROM csv_info WHERE date>= '".$start_date."' AND date<= '".$end_date."' GROUP BY project";
			  ...
			  while ($row = mysqli_fetch_assoc($result))
			    {
					...
					echo "<tr class='slidedown'>";
				  	$query = "SELECT distinct(user_name) FROM csv_info WHERE project='".$row['project']."'AND date>= '".$start_date."' AND date<= '".$end_date."'";
				  	...
				  	if ($res = mysqli_query($con, $query)){
				 		...
				 		<th>Total Time <small> in Hr</small></th>
							...
							while ($r = mysqli_fetch_assoc($res)){
								...
								$sql1= "SELECT SUM(Left(time,2) * 3600 + substring(time, 4,2) * 60 )AS time FROM csv_info WHERE project='".$row['project']."'AND date>= '".$start_date."' AND date<= '".$end_date."'AND user_name='".$r['user_name']."'";
								if ($result1 = mysqli_query($con, $sql1)){
									while ($ro = mysqli_fetch_assoc($result1)){
											$seconds = $ro['time'];
											$H = floor($seconds / 3600);
		...
		if ($result = mysqli_query($con, $sql)){
			...
			while ($row = mysqli_fetch_assoc($result)){
					...
					echo "<tr class='slidedown'>";
				  	$query = "SELECT distinct(user_name) FROM csv_info WHERE project='".$row['project']."'AND date>= '".$start_date."' AND date<= '".$end_date."'";
				  	if ($res = mysqli_query($con, $query)){
				  		echo "<td colspan='55'><table width='100%' bgcolor='#c0edea'>";
				  		echo "<tr bgcolor='#6b98e0'>
				 		<th>User</th>
				 		<th>Total Time <small> in Hr</small></th>
				 		</tr>";
		...
		$end_month = $d["month"];
	 	 ...
	 	 $sql = "SELECT project,
				    ...
				    if ($result = mysqli_query($con, $sql)){
						...
						$MonthArr = str_replace(10,'ten',$MonthArr);
						$MonthArr = str_replace(11,'el',$MonthArr);
						$MonthArr = str_replace(12,'tw',$MonthArr);
						// print_r($MonthArr);
				    	...
				    	echo "
							...
							<th>More</th>
							<th>Project</th>
							<th id='1'><small style='opacity: 0.0;'>1</small> January <br><small>In hours</small></th>
							<th id='2'><small style='opacity: 0.0;'>2</small> February <br><small>In hours</small></th>
							<th id='3'><small style='opacity: 0.0;'>3</small> March <br><small>In hours</small></th>
							...
							<th id='6'><small style='opacity: 0.0;'>6</small> June <br><small>In hours</small></th>
							<th id='7'><small style='opacity: 0.0;'>7</small> July <br><small>In hours</small></th>
							<th id='8'><small style='opacity: 0.0;'>8</small> August <br><small>In hours</small></th>
							<th id='9'><small style='opacity: 0.0;'>9</small> September <br><small>In hours</small></th>
							<th id='10'><small style='opacity: 0.0;'>ten</small> October <br><small>In hours</small></th>
							<th id='11'><small style='opacity: 0.0;'>el</small> November <br><small>In hours</small></th>
							<th id='12'><small style='opacity: 0.0;'>tw</small> December <br><small>In hours</small></th>
...
<script type="text/javascript">
	$(function(){
		...
		document.getElementById('project_select').value = "<?php echo $_POST['project_select'];?>";
   ...
   //--------- for hiding the extra weeks which have value 0 in week view
